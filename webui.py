...
# IA-2024-01
from modules.auth import check_role, auth_dict  # , role_dict, active_user
...
# IA-2024-01
def get_admin_state(request: gr.Request):
    user_name = request.username
    user_role = check_role(user_name)
...
title = f'IALCHEMY Image Generator'
...
with shared.gradio_root as app:
    # IA-2024-01
    m = gr.Markdown()
    logout_button = gr.Button("Logout", link="/logout")
    app.load(update_message, None, m)

    # IA-2024-01
    admin_state = gr.State(False)  # Initial state


    def update_admin_state(request: gr.Request):
        admin = get_admin_state(request)
        admin_state.value = admin
        print('state', admin)
        return admin


    app.load(update_admin_state, inputs=None, outputs=admin_state)


    def update_visibility(admin):
        return gr.update(visible=admin)
        ...
        with gr.Column(scale=2):
            with gr.Tab("Generation"):
                ...
                gallery = gr.Gallery(label='Gallery', show_label=False, object_fit='contain', visible=True, height=768,
                                     ...
                                     value=["assets/IA-preview.png"],
                                     preview=True)

            # IA-2024-01
            print('check admin state: ', admin_state.value)
            if admin_state.value:
                ...
                with gr.Tab("rembg"):
                    ...
                    gr.Markdown(
                        "Powered by [🪄 rembg 2.0.53](https://github.com/danielgatis/rembg/releases/tag/v2.0.53)")
                ...
                with gr.Tab("Online"):
                    with gr.Tab("Demos"):
                        for name in load_demos_names():
                            ...
                            with gr.Tab(name):
                                gr.HTML(
                                    f"<iframe src='{url}' width='100%' height='1080px' style='border-radius: 8px;'></iframe>")
                    with gr.Tab("Tools"):
                            ...
                            for _ in range(flags.controlnet_image_count):
                                with gr.Column():
                                    ...
                                    with gr.Column(visible=False) as ad_col:
                                        ...
                                        ip_type = gr.Radio(label='Type', choices=flags.ip_list_mod,
                                                           value=flags.default_ip, container=False)
                        ...
                        with gr.Row():
                            ...
                            with gr.Column(visible=False) as inpaint_mask_generation_col:
                                ...
                                inpaint_mask_cloth_category = gr.Dropdown(label='Cloth category',
                                                                          ...
                                                                          visible=False)
                                inpaint_mask_sam_prompt_text = gr.Textbox(label='Segmentation prompt', value='',
                                                                          visible=False)
                                with gr.Accordion("Advanced options", visible=False,
                                                  open=False) as inpaint_mask_advanced_options:
                                    inpaint_mask_sam_model = gr.Dropdown(label='SAM model',
                                    ...
                                    inpaint_mask_box_threshold = gr.Slider(label="Box Threshold", minimum=0.0,
                                                                           maximum=1.0, value=0.3, step=0.05)
                                    inpaint_mask_text_threshold = gr.Slider(label="Text Threshold", minimum=0.0,
                                                                            maximum=1.0, value=0.25, step=0.05)
                                generate_mask_button = gr.Button(value='Generate mask from image')


                                def generate_mask(image, mask_model, cloth_category, sam_prompt_text, sam_model,
                                                  sam_quant, box_threshold, text_threshold):
                                ...
                                inpaint_mask_model.change(
                                    lambda x: [gr.update(visible=x == 'u2net_cloth_seg'), gr.update(visible=x == 'sam'),
                                               gr.update(visible=x == 'sam')],
                                    inputs=inpaint_mask_model,
                                    outputs=[inpaint_mask_cloth_category, inpaint_mask_sam_prompt_text,
            ...
            if admin_state.value:
                ...
                with gr.Tab(label='Models'):
                    with gr.Group():
                        ...
                        refiner_model.change(lambda x: gr.update(visible=x != 'None'),
                                             inputs=refiner_model, outputs=refiner_switch, show_progress=False,
                                             queue=False)
                ...
                with gr.Tab(label='Advanced'):
                    ...
                    if os.path.exists(notification_file):
                        ...
                        play_notification.change(fn=play_notification_checked, inputs=[play_notification, notification],
